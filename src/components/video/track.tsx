...
export function VideoTrackView({
  ...
  const mediaUrl = media?.file_path ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${media.file_path}` : "";
  ...
  const handleResize = async (
    ...
    const handleMouseMove = async (moveEvent: MouseEvent) => {
      ...
      const nextFrameElement =
        trackElement.nextElementSibling as HTMLElement | null;
